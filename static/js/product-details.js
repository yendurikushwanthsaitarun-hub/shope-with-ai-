...
const ProductDetails = {
  ...
  toggleCompare(productId) {
    if (this.comparisonIds.has(productId)) {
      ...
      if (window.showToast) window.showToast('Removed from comparison list', 'info');
    } else {
      if (this.comparisonIds.size >= 4) {
        if (window.showToast) {
          window.showToast('You can compare a maximum of 4 products at a time.', 'warning');
        }
        return;
      }
      this.comparisonIds.add(productId);
      if (window.showToast) window.showToast('Added to comparison list', 'success');
    }
  ...
  async openComparisonModal() {
    if (this.comparisonIds.size === 0) {
      if (window.showToast) {
        window.showToast('Please check the "Compare" box on at least 1 or 2 products first.', 'info');
      }
      return;
    }
